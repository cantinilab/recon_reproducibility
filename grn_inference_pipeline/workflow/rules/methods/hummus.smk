...
rule download_granges:
    ...
    shell:
        """
        Rscript workflow/scripts/methods/hummus/get_granges.R {output.h} {output.m}
        """
...
rule prior_hummus:
    ...
    output:
        ...
        loom_rna=temp(local('datasets/{dataset}/cases/{case}/runs/{pre}.rna.loom')),
        ...
        hummus_object=local('datasets/{dataset}/cases/{case}/runs/{pre}.hummus_object.RDS'),
    params:
        organism=lambda w: config['datasets'][w.dataset]['organism'],
        grn_number_edges=lambda w: config['datasets'][w.dataset]['cases'][w.case].get('GRNBoost2_nlinks', 500000),
        tf_layer_method=None,
        n_cores=45,
        seed=lambda w: config['datasets'][w.dataset]['cases'][w.case].get('seed', 666)
    singularity:
        'workflow/envs/hummus.sif'
#    resources:
#       mem_mb=restart_mem,
...
rule p2g_hummus:
    ...
    #    multilayer_f=directory('datasets/{dataset}/cases/{case}/runs/{pre}.multilayer'),
    params:
        organism=lambda w: config['datasets'][w.dataset]['organism'],
        ext=500,
        n_cores=32
...
rule tfb_hummus:
    ...
    output:
        ...
        multilayer_f=directory('datasets/{dataset}/cases/{case}/runs/{pre}.{p2g}.multilayer'),
    params:
        organism=lambda w: config['datasets'][w.dataset]['organism'],
        n_cores=32,
        p2g=lambda w: w.p2g
...
rule mdl_hummus:
    ...
    output:
        ...
        multilayer_f=directory('datasets/{dataset}/cases/{case}/runs/{pre}.{p2g}.{tfb}.multilayer'),
    params:
        n_cores=32,
        organism=lambda w: config['datasets'][w.dataset]['organism'],
        p2g=lambda w: w.p2g,
...
rule mdl_o_hummus:
    ...
    params:
        organism=lambda w: config['datasets'][w.dataset]['organism'],
        grn_number_edges=lambda w: config['datasets'][w.dataset]['cases'][w.case].get('GRNBoost2_nlinks', 50000),
        tf_layer_method=None,
        ext=500,
        n_cores=16,
    singularity:
